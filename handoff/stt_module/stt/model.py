...
class ModelManager:
    ...
    def get_model(cls) -> WhisperModel:
        ...
        with cls._lock:
            ...
            try:
                # import까지 지연해 설정 확인이나 다른 모듈 사용만으로는 무거운
                # faster-whisper 런타임을 초기화하지 않도록 한다.
                from faster_whisper import WhisperModel
                ...
                # 원본 예외를 연결해 호출자가 상세 원인을 추적할 수 있게 하면서,
                # 어떤 설정으로 로딩하다 실패했는지 즉시 파악할 메시지를 제공한다.
                raise RuntimeError(
    ...
    def unload_model(cls) -> None:
        ...
        with cls._lock:
            ...
            # 클래스 참조를 먼저 제거해 이후 모델 요청이 새로운 인스턴스를
            # 생성할 수 있는 명확한 미로딩 상태로 전환한다.
            model = cls._model
            cls._model = None
