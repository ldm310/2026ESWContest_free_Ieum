...
## 13. BEM·MVDR·CTranslate2 같은 전문용어가 잡음에서 반복됨

실시간 모델에 `hotwords`를 강제로 전달하는 이전 코드이거나, 시작 1초 잡음
보정 전에 말했을 가능성이 큽니다. 현재 코드에서는 실시간 `hotwords`를 모델에
전달하지 않고 Final VAD·신뢰도·환각 필터를 적용합니다.

- `start()` 후 1초간은 말하지 않고 배경음만 입력하십시오.
- Beamforming mono를 16 kHz, float32, `-1.0~1.0`으로 전달하십시오.
- 최신 코드에서 같은 현상이 재현되면 무음 RMS·peak·클리핑 비율을 먼저
  측정하십시오.
