#!/usr/bin/env python3
"""Jetson 1024×600 모니터용 실시간 화자 인식 자막 UI 서버.

실제 실행에서는 ``realtime_doa.py``가 전송한 방향과 mono 오디오를 받고,
카메라 얼굴 위치와 결합해 화자별 partial/final 자막을 브라우저 UI에 제공한다.
``--demo`` 모드는 Jetson 장치 없이 화면과 상태 갱신을 검증할 때 사용한다.
"""
...
class RuntimeState:
    """카메라·DOA·STT 스레드와 HTTP 요청이 공유하는 상태."""
    ...
    def set_status(self, **changes: Any) -> None:
        """연결 및 실행 상태를 원자적으로 갱신한다."""

        with self._lock:
            self._status.update(changes)

    def toggle(self, key: str) -> bool:
        """recording 또는 diarization 상태를 반전한다."""

        if key not in {"recording", "diarization"}:
...
class StreamingSTTService:
    """StreamingSTT의 시작·입력·종료를 UI 생명주기에 맞춘다."""
    ...
    def start(self) -> None:
        """Whisper 모델과 StreamingSTT worker를 시작한다."""
    ...
    def push_audio(self, audio: Any) -> None:
        """빔포밍 mono 오디오를 StreamingSTT에 전달한다."""
...
class RealtimeReceiver:
    """DOA 프로세스의 방향과 mono 오디오 UDP 패킷을 수신한다."""
...
class OpenCVFaceDetector:
    """Jetson 기본 OpenCV에서 동작하는 경량 얼굴 검출기."""
...
class CameraWorker:
    """RealSense 우선, 일반 USB 카메라 fallback으로 영상과 얼굴 위치를 갱신한다."""
    ...
    def _realsense_face_direction(
        ...
        # 기존 박스 좌표계: 카메라 Z→마이크 X, 카메라 X→마이크 Y.
        vector = np.array(
...
class DemoEngine:
    """장치 없이 UI 갱신과 화면 크기를 검증하는 상태 생성기."""
...
class CaptionHTTPServer(ThreadingHTTPServer):
    """각 MJPEG 연결을 별도 daemon 스레드에서 처리하는 HTTP 서버."""
...
class UIRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            value = self.server.state.toggle(str(payload.get("action", "")))
...
def open_kiosk(url: str) -> subprocess.Popen[Any] | None:
    """Jetson Chromium을 1024×600 전체 화면 앱 모드로 연다."""
...
def validate_runtime_files() -> None:
    """Jetson에 복사할 때 빠지기 쉬운 UI 파일을 시작 전에 검증한다."""
...
def main() -> None:
    """상태 서비스와 로컬 UI 서버를 시작하고 종료 시 모든 자원을 정리한다."""
