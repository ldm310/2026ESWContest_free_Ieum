...
const elements = {
  ...
  diarizationButton: document.getElementById("diarization-button"),
...
function createSpeakerTag(speakers, speakerId) {
  ...
  tag.style.backgroundColor = SPEAKER_COLORS[speakerId] || "#667085";
  return tag;
}
...
function renderFaces(state) {
  ...
  state.faces.forEach((face) => {
    ...
    box.className = `face-box${face.active ? " active" : ""}`;
...
function renderCaptions(state) {
  ...
  captions.forEach((caption) => {
    ...
    row.appendChild(createSpeakerTag(state.speakers, Number(caption.speaker_id)));
  ...
  if (state.partial?.text) {
    ...
    row.appendChild(createSpeakerTag(state.speakers, speakerId));
...
function renderControls(status) {
  ...
  const diarization = Boolean(status.diarization);
  ...
  elements.recordLabel.textContent = recording ? "녹음 중" : "녹음 시작";
  elements.diarizationToggle.classList.toggle("on", diarization);
  elements.diarizationButton.setAttribute("aria-pressed", String(diarization));
}
...
elements.recordButton.addEventListener("click", () => toggleControl("recording"));
elements.diarizationButton.addEventListener("click", () => toggleControl("diarization"));
